...
function po_admin_print_scripts()
{
  ...
  if ('about' === $post_type || 'faq' === $post_type) {
    wp_enqueue_script('jquery-ui-sortable');
    wp_enqueue_script('birth-post-order', get_template_directory_uri() . '/admin/js/post-order.min.js', array('jquery', 'jquery-ui-sortable'), VERSION, true);
...
function change_post_order()
{
  ...
  if ('about' !== $post_type && 'faq' !== $post_type) return;
...
function po_pre_get_posts($query)
{
  ...
  if (is_admin()) return;
  if ($query->is_post_type_archive('about') || $query->is_post_type_archive('faq')) {
