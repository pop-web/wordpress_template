...
if ( ! function_exists( 'minnanowordpress_setup' ) ) :
	...
	function minnanowordpress_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'minnanowordpress' ),
			)
		);
...
function minnanowordpress_scripts() {
	wp_enqueue_style( 'minnanowordpress-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'minnanowordpress-style', 'rtl', 'replace' );

	wp_enqueue_script( 'minnanowordpress-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
